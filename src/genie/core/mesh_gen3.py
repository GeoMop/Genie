...
def gen(gallery_mesh_file, out_brep_file, mesh_cut_tool_param, inv_par, project_conf):
    ...
    for i in [4]:#range(6):
        ...
        for j in range(len(face_edges[i])):
            for e in face_edges[i][j]:
                try:
                    bw_edges[e].attach_to_plane(surf, vertices_tr[e[0]][1:], vertices_tr[e[1]][1:])
                except KeyError:
                    bw_edges[(e[1], e[0])].attach_to_plane(surf, vertices_tr[e[1]][1:], vertices_tr[e[0]][1:])
    ...
    for i in [3]:#range(6):
        ...
        e2.attach_to_plane(surf, (1, 0), (1, 1) )
        e11.attach_to_plane(surf, (1, 1), (0, 1) )
        e6.attach_to_plane(surf, (0, 0), (0, 1) )
        e10.attach_to_plane(surf, (1, 0), (0, 0) )

        for j in range(len(face_edges[i])):
            for e in face_edges[i][j]:
                try:
                    bw_edges[e].attach_to_plane(surf, vertices_tr[e[0]][:2], vertices_tr[e[1]][:2])
                except KeyError:
                    bw_edges[(e[1], e[0])].attach_to_plane(surf, vertices_tr[e[1]][:2], vertices_tr[e[0]][:2])
