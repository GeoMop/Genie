...
!define MULTIUSER_INSTALLMODE_INSTDIR "GenieST"
...
OutFile "${GIT_DIR}\dist\genie_st_${VERSION}_x86_64.exe"

# Registry key to check for directory (so if you install again, it will 
# overwrite the old one automatically)
InstallDirRegKey HKCU "Software\GenieST" "Install_Dir"
...
Function .onInit
  ...
  !define APP_HOME_DIR "$APPDATA\GenieST"
...
Section "Genie" SecGenie
  ...
  SetOutPath $INSTDIR
  File /r /x *~ /x __pycache__ /x pylintrc /x *.pyc /x genie_rt.py "${SRC_DIR}\genie"
...
Section "Start Menu shortcuts" SecStartShortcuts
  ...
  CreateDirectory "$SMPROGRAMS\GenieST"

  # Make sure this is clean and tidy.
  RMDir /r "$SMPROGRAMS\GenieST"
  CreateDirectory "$SMPROGRAMS\GenieST"
  
  # Uninstall shortcut.
  SetOutPath $INSTDIR
  CreateShortcut "$SMPROGRAMS\GenieST\Uninstall.lnk" "$INSTDIR\uninstall.exe" "" "$INSTDIR\uninstall.exe" 0

  IfFileExists "$INSTDIR\genie\genie_ert.py" 0 +3
    SetOutPath $INSTDIR\genie
    CreateShortcut "$SMPROGRAMS\GenieST\GenieERT.lnk" "$INSTDIR\bin\pythonw.bat" '"$INSTDIR\genie\genie_ert.py"' "$INSTDIR\genie\icons\genie_ert_128.ico" 0

  IfFileExists "$INSTDIR\genie\genie_st.py" 0 +3
    SetOutPath $INSTDIR\genie
    CreateShortcut "$SMPROGRAMS\GenieST\GenieST.lnk" "$INSTDIR\bin\pythonw.bat" '"$INSTDIR\genie\genie_st.py"' "$INSTDIR\genie\icons\genie_st_128.ico" 0
...
Section -post
  ...
  WriteRegStr HKCU SOFTWARE\GenieST "Install_Dir" "$INSTDIR"
  
  ; Write the uninstall keys for Windows
  WriteRegStr HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\GenieST" "DisplayName" "GenieST"
  WriteRegStr HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\GenieST" "UninstallString" "$\"$INSTDIR\uninstall.exe$\""
  WriteRegDWORD HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\GenieST" "NoModify" 1
  WriteRegDWORD HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\GenieST" "NoRepair" 1
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\GenieST"
  DeleteRegKey HKCU SOFTWARE\GenieST
